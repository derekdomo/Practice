...
where edges[i] = [ai, bi] means that exists an edge connecting the vertices ai 
and bi. Additionally, there is a boolean array hasApple, 
...
class Solution:
    def minimumTimeCollectApples(self, n, edges, hasApple):
        adj = defaultdict(list)
        for edge in edges:
            adj[edge[0]].append(edge[1])
            adg[edge[1]].append(edge[0])

        target = set()
        for i, n in enumerate(hasApple):
            if n:
                target.add(i)
        ...
        def dfs(adj, start, target, visited):
            if start in visited:
                return 0
            if start in target:
                target.remove(start)
            ...
            visited.add(start)
            for nb in adj[start]:
                steps += dfs(adj, nb, target, visited)
